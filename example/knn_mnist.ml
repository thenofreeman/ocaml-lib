...
let load_mnist image_file label_file =
  ...
  let features = Matrix.of_array n_images (n_rows * n_cols) image_data in
  let labels = Vector.of_array (Array.map float_of_int labels) in
  (features, labels)
...
let () =
  ...
  let n = 10 in
  for i = 0 to n-1 do
    let query = Matrix.row x_test i in
